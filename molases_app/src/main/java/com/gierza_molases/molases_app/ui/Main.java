...
public class Main extends JFrame {
	...
	private JPanel createSidebar() {
		...
		String[] navItems = {"Dashboard", "Delivery", "Customer", "Product", "Exit"};
	...
	private JPanel createNavButton(String text) {
		...
		if (text.equals("Dashboard")) {
			...
			if (iconPanel != null) iconPanel.setBackground(SIDEBAR_ACTIVE);
		}
		...
		button.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				if (!text.equals(currentPage)) {
					...
					if (iconPanel != null) iconPanel.setBackground(SIDEBAR_HOVER);
				}
			}
			...
			public void mouseExited(MouseEvent e) {
				if (!text.equals(currentPage)) {
					button.setBackground(SIDEBAR_BG);
					iconWrapper.setBackground(SIDEBAR_BG);
					if (iconPanel != null) iconPanel.setBackground(SIDEBAR_BG);
				}
			}
	...
	private void showPage(String pageName) {
		...
		// Check if it's the Customer page
		if (pageName.equals("Customer")) {
			// Load the CustomersPage panel
			mainContentArea.add(molases_appcom.gierza_molases.molases_app.ui.pages.CustomersPage.createPanel(), BorderLayout.CENTER);
		} else {
	...
	private class DeliveryLogoIcon extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			g2d.setStroke(new java.awt.BasicStroke((float)(2.5 * scale), 
