...
public class AddCustomerDialog extends JDialog {
	...
	private JPanel createButtonPanel() {
		...
		// Save button
		JButton saveBtn = createStyledButton("Save Customer", ACCENT_GOLD);
	...
	private boolean validateAndSave() {
		...
		if (!hasCustomerAddressBranch && branches.isEmpty()) {
			ToastNotification.showError(this, "At least one branch is required");
			return false;
		}
		...
		try {
			if (isIndividual) {
				customerService.addCustomerAsIndividualType(firstName, middleName, lastName, contactNumber, address,
						branches);
			} else {
				customerService.addCustomerAsCompanyType(companyName, contactNumber, address);
			}
			return true;
		} catch (Exception e) {
			ToastNotification.showError(this, "Failed to save customer: " + e.getMessage());
			e.printStackTrace();
			return false;
		}
	}
